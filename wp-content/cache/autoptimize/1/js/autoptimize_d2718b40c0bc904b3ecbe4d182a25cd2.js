    ...
    $(function() {
        ...
        if (!$('body').hasClass('body-style-scroll')) {
            $('.anchor-link').on('click', function() {
                ...
                $(this).closest('.card-wrap').stop().animate({
                    scrollTop: id_url
                });
                return false;
            });
        ...
        if ($('body').hasClass('home') && $('.top-menu').hasClass('top-menu-onepage')) {
            $('.post-password-form').on('submit', function() {
                Cookies.set('submit-post-password', $(this).closest('.card-inner').attr('id'), {
                    expires: 7,
                    path: '/'
                });
                $(this).submit();
            ...
            if (post_password_cookie !== undefined) {
                ...
                Cookies.remove('submit-post-password', {
                    path: '/'
                });
            }
        }
...
(function($) {
    ...
    var demo = ['digital', 'developer', 'designer', 'system', 'dark', 'business', 'marketing', 'streamer', 'writer', 'crypto', 'lawyer', 'seo', 'cybersecurity', 'ui-designer', 'app-developer', 'dataops', 'game-developer', 'project-manager', 'ml-engineer', 'rtl'];
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $container.find('.content').html('<p class="error">' + (response.data.message || 'Loading failed.') + '</p>');
                        }
                    },
                    error: function() {
                        $container.find('.content').html('<p class="error">Request failed. Please try again.</p>');
                    }
                });
            },
            open: function() {}
